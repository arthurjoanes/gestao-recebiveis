...
1. **Resumo (30s).** Mostre o aviso de mensagens simuladas e a data comercial. Explique que aberto/vencido filtram vencimento, enquanto recebido filtra data de pagamento. Abra a composição do aberto: vencido mais em dia (inclui hoje). Use **Ver títulos vencidos** para conferir o recorte. Os filtros ficam após os saldos; **Hoje**, **Este mês**, **Personalizado** e **Limpar filtros** controlam o período. Inverta um intervalo para mostrar o erro junto ao campo, preservando o resultado anterior. No celular, use **Menu** para acessar todas as áreas. Toda informação financeira vem do banco.
