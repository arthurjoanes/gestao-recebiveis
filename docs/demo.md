...
1. **Resumo (30s).** Mostre a identificação da demonstração e a data comercial. O saldo e os títulos vencidos vêm primeiro; **Vencimentos e recebimentos** abre a composição e os pagamentos do período. Explique que aberto/vencido filtram vencimento, enquanto recebido filtra data de pagamento. Use **Ver títulos vencidos** para conferir o recorte. Os filtros ficam acima do livro financeiro; **Hoje**, **Este mês**, **Personalizado** e **Limpar filtros** controlam o período. Inverta um intervalo para mostrar o erro junto ao campo, preservando o resultado anterior. No celular, use **Menu** para acessar todas as áreas. Toda informação financeira vem do banco.
